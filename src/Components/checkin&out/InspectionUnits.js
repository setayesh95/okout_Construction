...
function InspectionUnits({ navigation, navigation: { goBack } }) {
  ...
  const renderItem = ({ item, index }) => (
    <Doc_List_Item key={index}  value={item} SeeDetail={SeeDetail} categorylist={categorylist} Screen={'InspectionUnits'}
  ...
  const getUnits=async ()=>{
    ...
    let getUnits = [];
    let Filter_Json=json.find((p)=>p.siteId===GLOBAL.UnitId)?.units;
    Filter_Json?.forEach((obj) => {
      getUnits.push({
        Id: obj?.unitId,
        name: obj?.unitName,
        unitSiteId:obj?.unitSiteId,
        unitSiteName:obj?.unitSiteName,
        unitTypeName:obj?.unitTypeName,
        unitDescription:obj?.unitDescription,
        address:obj?.address,
        geoLat:obj?.geoLat,
        geoLong:obj?.geoLong,
        notes:obj?.unitDescription,
        task:obj?.task,
  ...
  return (
    <Container  style={{backgroundColor:GLOBAL.backgroundColor}}>
      <Header colors={["#59a890", "#417f73", "#0B2B26"]} StatusColor={"#58a78f"} onPress={goBack} Title={"Units"} />
...
export default InspectionUnits;
